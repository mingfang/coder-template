    ...
    template {
      ...
      spec {
        ...
        container {
          ...
          command           = ["sh", "-c", <<-EOF
            ${replace(coder_agent.pod.init_script, data.coder_workspace.me.access_url, var.coder_access_url)}
            EOF
          ]
        ...
        # vscode
        dynamic "container" {
          for_each = data.coder_parameter.vscode.value ? {"test"="test"} : {}
          content {
            name    = "vscode"
            image   = "codercom/code-server:latest"              
            args = ["--auth", "none", "--port", "13337"]
